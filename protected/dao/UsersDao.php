...
class UsersDao{
	 ...
	 * @throws Exception
	 */
 	public function exitsUserInDataBase($email){//Ya esta optimizado
 	 ...
 	 * @param int $idusers
 	 */
 	public function updatePasswordForUser($password,$idusers){//Ya esta optimizado
 	 ...
 	 * @return boolean
 	 */
 	public function exitsCodeInUsers($codeId){
 		$connection=Yii::app()->db;
	 ...
	 * Registra en la base de datos un nuevo usuario asociado a un codigo
 	 ...
 	 * @throws Exception
 	 */
 	public function validActivationCodeInDataBase($activationCode){//*
 		$connection=Yii::app()->db;
	 ...
	 */
 	 ...
 	 */
 	public function activateAccount($activationCode){//Ya esta optimizado
 	 ...
 	 * @return string
 	 */
 	private function krypPassword($password){
	 ...
	 */
 	...
 	public function updateUserData($model){
 		$command = Yii::app()->db->createCommand();
 		//$command->update('users', array( 'password'=>self::krypPassword($password), 'change_password_code'=>'','authToken'=>''), 'idusers=:id', array(':id'=>$idusers));
