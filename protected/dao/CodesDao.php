...
class CodesDao{
	...
	public function exitsCodeInDataBase($code){
 	 ...
 	 * @return int
 	 */
 	public function getCodeId($code){//Ya esta optimizado
 		$idCode = 0;
 		$connection=Yii::app()->db;
 		$sql = Querys::SEARCH_CODE;
 		$command = $connection->createCommand($sql);
 	 ...
 	 * @return boolean
 	 */
 	 public function registerCodeIntoDataBase($code,$duration){//Ya esta revisado
 	 	$registerCodeSuccess = false;
 	 	try{
	 	 	$sql = Querys::INSERT_CODE;
	 	 	$parameters = array(":code"=>$code, ':created' => date('Y-m-d H:i:s'), ':duration'=> $duration);
