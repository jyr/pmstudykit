...
class CodeGenerator{
	 ...
	 * @return string code
	 */
	public static function generatorCode($length,$duration = 0){
		$code =  $duration > 0 ? Constants::PREFIX_PROMO_CODE.self::generateCodeForToolkit($length) : self::generateCodeForToolkit($length) ;
		$registerCodeSuccess = CodesDao::getInstance()->registerCodeIntoDataBase($code,$duration);
